...
class FreightController {
    ...
    async handleAuthState(user) {
        ...
        if (user) {
            ...
            if (userInfo) {
                ...
                userInfo.innerHTML = `<div class="d-flex align-items-center gap-2" title="Meu Perfil"><div class="bg-primary rounded-circle text-white d-flex align-items-center justify-content-center shadow-sm" style="width:36px; height:36px; font-weight:bold;">${inicial}</div><div style="line-height:1.2;"><strong class="text-white">${nome}</strong><br><span class="text-white-50 x-small">${empresa} (${cargo})</span></div></div>`;
            }
            this.view.showToast(`Bem-vindo, ${user.name || user.displayName}!`);
            if (user.teamId) this.model.iniciarSincronizacao(user.uid, user.teamId);
    ...
    initProfileListeners() {
        const userInfo = document.getElementById('userInfoDisplay');
        if (userInfo) userInfo.addEventListener('click', () => { this.carregarDadosPerfil(); const modalEl = document.getElementById('profileModal'); if (modalEl) new bootstrap.Modal(modalEl).show(); });
        document.getElementById('btnSaveProfile')?.addEventListener('click', async () => { const user = this.authService.currentUser; const newPhone = document.getElementById('profPhone').value; const newJob = document.getElementById('profJobTitle').value; const newBranch = document.getElementById('profBranch').value; const newCompany = document.getElementById('profCompany').value; this.view.setLoading(true, "Atualizando..."); try { await this.model.atualizarPerfilUsuario(user.uid, { phone: newPhone, jobTitle: newJob, branch: newBranch, company: newCompany }, user.teamId); this.authService.currentUser.phone = newPhone; this.authService.currentUser.jobTitle = newJob; this.authService.currentUser.branch = newBranch; this.authService.currentUser.company = newCompany; this.view.setLoading(false); this.view.showToast("Perfil atualizado!"); this.handleAuthState(this.authService.currentUser); } catch (e) { this.view.setLoading(false); alert("Erro: " + e.message); } });
    ...
    carregarDadosPerfil() {
        ...
        const elCompany = document.getElementById('profCompany'); if (elCompany) { if (user.role === 'Admin') { elCompany.removeAttribute('disabled'); elCompany.classList.add('border-primary'); } else { elCompany.setAttribute('disabled', 'true'); elCompany.classList.remove('border-primary'); } }
        const selectBranch = document.getElementById('profBranch'); if (selectBranch) { selectBranch.innerHTML = '<option value="">Selecione uma filial...</option>'; if (this.model.locaisSalvos) this.model.locaisSalvos.forEach(l => { const opt = document.createElement('option'); opt.value = l.nome; opt.textContent = l.nome; if (user.branch === l.nome) opt.selected = true; selectBranch.appendChild(opt); }); }
        const elAccess = document.getElementById('profAccessLevel'); if (elAccess) { const role = user.role ? user.role.toUpperCase() : 'MEMBRO'; elAccess.value = role; elAccess.className = `form-control fw-bold ${role === 'ADMIN' ? 'text-danger' : 'text-success'}`; }
        const teamSection = document.getElementById('teamManagementSection'); if (teamSection) { if (user.role === 'Admin') { teamSection.classList.remove('d-none'); this.renderizarListaMembros(); } else { teamSection.classList.add('d-none'); } }
    }

    renderizarListaMembros() {
        const list = document.getElementById('teamMembersList'); if (!list) return; const members = this.model.teamMembers || []; if (!members.length) { list.innerHTML = '<div class="text-center p-3 text-muted small">Nenhum membro.</div>'; return; }
        list.innerHTML = members.map(m => `<div class="list-group-item d-flex justify-content-between align-items-center"><div><strong>${m.name}</strong> <span class="badge bg-light text-dark border ms-1">${m.jobTitle || 'Sem cargo'}</span><br><small class="text-muted">${m.email}</small> <span class="text-primary x-small fw-bold">(${m.role})</span></div><div class="d-flex gap-1"><button class="btn btn-outline-primary btn-sm btn-edit-member" data-email="${m.email}"><i class="fas fa-pencil-alt"></i></button><button class="btn btn-outline-danger btn-sm btn-remove-member" data-email="${m.email}"><i class="fas fa-times"></i></button></div></div>`).join('');
        list.querySelectorAll('.btn-remove-member').forEach(btn => btn.onclick = async () => { if (confirm(`Remover?`)) { const mem = members.find(x => x.email === btn.dataset.email); if (mem) await this.model.removerMembroEquipe(mem); } });
        list.querySelectorAll('.btn-edit-member').forEach(btn => btn.onclick = () => { const mem = members.find(x => x.email === btn.dataset.email); if (mem) { document.getElementById('editMemberEmail').value = mem.email; document.getElementById('editMemberNameDisplay').value = mem.name; document.getElementById('editMemberJobTitle').value = mem.jobTitle || ''; document.getElementById('editMemberRole').value = mem.role || 'Operacional'; new bootstrap.Modal(document.getElementById('editMemberModal')).show(); } });
    }

    // --- INICIALIZAÇÃO ---
    init() {
        document.addEventListener('dataSynced', () => { this.view.renderizarListaLocais(this.model.locaisSalvos); this.view.preencherConfig(this.model.configGlobal, this.model.frotaDefault); const modal = document.getElementById('profileModal'); if (modal && modal.classList.contains('show')) this.renderizarListaMembros(); this.view.showToast("Dados atualizados."); });
        this.view.setupSobre();
        const tabEls = document.querySelectorAll('button[data-bs-toggle="tab"]'); tabEls.forEach(tab => { tab.addEventListener('shown.bs.tab', event => { const target = event.target.getAttribute('data-bs-target'); if (target === '#data-pane') document.querySelector('.main-layout').classList.add('sidebar-full-width'); else { document.querySelector('.main-layout').classList.remove('sidebar-full-width'); setTimeout(() => { if (this.view.map) this.view.map.invalidateSize(); }, 300); } }); });
        ...
        document.getElementById('btnResetConfig')?.addEventListener('click', async () => { if (await this.view.showConfirm("Recarregar dados?")) location.reload(); });
        document.getElementById('btnAddLoc')?.addEventListener('click', async () => { const nome = document.getElementById('newLocName').value; const link = document.getElementById('newLocLink').value; let coords = this.model.extrairCoordenadas(link); if (!coords) { this.view.setLoading(true, "Buscando..."); coords = await this.model.buscarCoordenadas(link); this.view.setLoading(false); } if (coords) { await this.model.salvarLocal(nome, coords); this.view.showToast("Salvo!"); document.getElementById('newLocName').value = ''; document.getElementById('newLocLink').value = ''; } else { this.view.showModal("Endereço inválido."); } });
        document.getElementById('savedLocationsList')?.addEventListener('click', async (e) => { const btn = e.target.closest('.btn-remove-loc'); if (btn && confirm("Excluir local?")) await this.model.removerLocal(parseInt(btn.dataset.id)); });
        ...
        // EXCEL (CORRIGIDO LOADING)
        document.getElementById('excelInput')?.addEventListener('change', async e => {
            if (!e.target.files.length) return;
            try {
                this.view.setLoading(true, '<h3 class="text-white">Processando Planilha...</h3>');
                ...
                this.view.setLoading(true, '<h3 class="text-white">Geocodificando Endereços...</h3>');
                ...
                this.pedidosGeocodificados = await this.model.geocodificarLote(this.todosPedidos, pct => {
                    this.view.setLoading(true, `<h1 class="display-1 text-white fw-bold">${pct}%</h1><p class="text-white fs-5">Geocodificando...</p>`);
                });
                if (this.pedidosGeocodificados.length > 0 && this.model.salvarAprendizado) { await this.model.salvarAprendizado(); }
        ...
        const dragbar = document.getElementById('dragbar'); const sidebar = document.getElementById('sidebar'); let isResizing = false;
        if (dragbar && sidebar) {
    ...
    async processar() {
        ...
        const updateStatus = (pct, msg) => {
            this.view.setLoading(true, `<h1 class="display-1 text-white fw-bold">${pct}%</h1><p class="text-white fs-5">${msg}</p>`);
        };

        try {
            updateStatus(0, "Iniciando...");
            ...
            this.resultadoCache = await this.model.gerarViagensInteligentes(origem, this.pedidosGeocodificados, frota, conf, pct => {
                const globalPct = Math.round((pct / 100) * 40);
                updateStatus(globalPct, "Criando rotas...");
            });
            ...
            const totalViagens = this.resultadoCache.viagens.length;
            const messages = ["Calculando percurso...", "Verificando pedágios...", "Encontrando rota retorno..."];

            if (totalViagens > 0) {
                for (let i = 0; i < totalViagens; i++) {
                    const basePct = 40 + Math.round((i / totalViagens) * 60);
                    const msg = messages[i % messages.length];
                    updateStatus(basePct, `${msg}<br><span class="fs-6 text-white-50">Rota ${i + 1}/${totalViagens}</span>`);
    ...
    // --- LÓGICA DE APRENDIZADO NA TROCA DE VEÍCULO ---
        ...
        if (e.target.classList.contains('select-vehicle-change')) {
            ...
            if (viagem && viagem.veiculo.tipo !== novoTipo) {
                ...
                if (novoVeiculoConfig) {
                    ...
                    setTimeout(async () => {
                        const isPaletizada = await this.view.showConfirm(`<div class="text-center"><i class="fas fa-pallet fa-3x text-primary mb-3"></i><br><h5>Aprendizado de Frota</h5><p>A carga é <b>Paletizada</b>?<br>Isso define preferência por <b>${novoTipo}</b>.</p></div>`);
                        let motivo = null;
                        if (isPaletizada) motivo = 'Carga Paletizada';
                        else {
                            const isExclusivo = await this.view.showConfirm(`<div class="text-center"><i class="fas fa-stopwatch fa-3x text-warning mb-3"></i><br><h5>Aprendizado de Frota</h5><p>É entrega <b>Agendada / Exclusiva</b>?<br>Isso fixa preferência por <b>${novoTipo}</b>.</p></div>`);
                        ...
                        if (motivo) {
                            ...
                            this.view.setLoading(false);
                            this.view.showToast(`Preferência salva.`);
                        }
                    }, 500);
                }
            }
